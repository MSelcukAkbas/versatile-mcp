...
class OrchestrationEngine:
    ...
    DETERMINISTIC_TOOLS = {
        "kubectl_exec", "search_files", "read_file", "list_directory",
        "search_semantic", "memory_retrieve_facts", "get_file_info", 
        "check_port", "system_info", "read_multiple_files",
        "directory_tree", "memory_search_semantic", "grep_search"
    }

    # High-risk tools that should NEVER auto-execute without explicit agent multi-step planning
    HIGH_RISK_TOOLS = {
        "write_file", "edit_file", "apply_patch", "run_command", "send_command_input",
    ...
    def analyze(self, thought: str, context: Dict[str, Any], available_tools: List[str], 
        ...
        for tool in available_tools:
            ...
            if tool in ["read_file", "view_file"] and any(w in thought_lower for w in ["file", "read", "oku", "dosya"]):
                tool_score[tool] += 2
            if tool in ["edit_file", "write_file", "apply_patch"] and any(w in thought_lower for w in ["change", "fix", "update", "güncelle", "yaz"]):
                tool_score[tool] += 2
                complexity = "medium"
        ...
        if best_tool in self.HIGH_RISK_TOOLS:
            risk_level = "high"
        elif best_tool in ["edit_file", "apply_patch", "run_command"]:
            risk_level = "medium"
